...
const urgencies = ["immediate", "within_24_hours", "within_week", "scheduled"];
...
async function seedPatients(count = 25) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const patientData = {
      ...
      p_city: city,
      p_bloodgroup: randomFrom(bloodGroups),
      ...
      p_urgencyRequirment: urgency,
      p_condition: randomFrom(["Anaemia", "Post-surgery recovery", "Trauma", "Tick fever", "Ehrlichiosis", "Bone marrow disease", "Internal bleeding"]),
      onboarded: "yes",
      role: "individual",
...
async function seedVeterinaries() {
  ...
  for (const clinic of vetClinics) {
    ...
    console.log(`  ✅ Vet: ${clinic.name} → ${email}`);
  }
}
...
async function main() {
  ...
  await seedVeterinaries();
  await seedDonations(40);
  await seedAppointments(20);

  console.log("\n✅ Done! 30 donors | 25 patients | 8 vet clinics | 40 donations | 20 appointments");
  process.exit(0);
}
